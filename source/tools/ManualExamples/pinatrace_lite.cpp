...
KNOB<string> KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool",
    "o", "pinatrace_lite.out", "specify trace file name");
...
static VOID EmitMem(VOID * ea, INT32 size)
{
    ...
    switch(size)
    {
      ...
      case 8:
        ...
        if (*static_cast<UINT64*>(ea) == 0xdeadbeef ) {
           if (!my_count) {
                magic_address = (UINT64)ea; 
                std::cout << "PIN: Got magic address!: 0x" << std::hex << magic_address << std::endl;
           }
           my_count++;
        }
        if (my_count == 16) {
            int i;
            for (i=0; i<16; i+=2) {  //zero out even entries of array
                *static_cast<UINT64*>((UINT64*)(magic_address+i*8)) = 0;
            }
        }
        break;
